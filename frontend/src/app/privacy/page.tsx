...
const policies = [
  ...
  },
  {
    ...
    metadata: { "Model Training": "Prohibited", "Anonymization": "Synthetic Only" }
  },
  {
      ...
      <div className="flex flex-1 w-full mx-auto relative overflow-hidden">
        ...
        <div className="flex-1 lg:ml-16 w-full px-4 sm:px-8 lg:px-8 flex justify-center py-6 md:py-8 h-full overflow-hidden">
          ...
          <div className="w-full flex flex-col lg:flex-row items-start gap-8 lg:gap-12 relative h-full">
            ...
            <div className="flex-1 w-full max-w-[800px] mx-auto min-w-0 h-full pt-4 lg:pt-0 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both" key={activePolicy.id}>
              ...
              <div className="mb-12">
                ...
                <div className="space-y-6">
                  {activePolicy.paragraphs.map((paragraph, idx) => (
                    <p key={idx} className="text-foreground/80 text-lg leading-loose">
                      {paragraph}
                    </p>
                  ))}
                </div>
            ...
            <aside className="hidden xl:flex flex-col w-80 shrink-0 h-full pl-4 pb-12 animate-in fade-in slide-in-from-right-8 duration-500 fill-mode-both" key={`meta-${activePolicy.id}`}>
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {activePolicy.tags.map((tag, idx) => (
                    <span key={idx} className="px-3 py-1.5 bg-brand-50 dark:bg-white/5 border border-brand-200 dark:border-white/10 text-brand-700 dark:text-brand-300 text-xs font-bold rounded-full shadow-sm">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="mb-8">
                ...
                <div className="bg-brand-50/30 dark:bg-white/5 border border-brand-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm">
                  {Object.entries(activePolicy.metadata).map(([key, value], idx, arr) => (
                    <div 
                      key={key} 
