...
export default function FieldHelper({ title }: FieldHelperProps) {
  return (
    <div className="relative inline-block ml-2 align-middle group cursor-help z-20">
      <div className="text-slate-400 dark:text-slate-500 hover:text-brand-600 transition-colors">
        <HelpCircle size={18} />
      </div>
      
      {/* Tooltip Card (hidden by default, shown on group-hover) */}
      <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3 w-80 sm:w-96 p-5 rounded-2xl bg-white dark:bg-slate-900 shadow-2xl shadow-slate-200/50 border border-slate-200 dark:border-slate-800 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 pointer-events-none transform translate-y-2 group-hover:translate-y-0 font-normal normal-case">
        ...
        <div className="flex items-center gap-2 text-[11px] font-bold text-brand-600 uppercase tracking-wider mb-2">
          <Sparkles size={14} />
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-slate-600">
            <li className="flex gap-2.5 items-start">
              <Scale className="text-slate-400 dark:text-slate-500 shrink-0 mt-0.5" size={15} />
